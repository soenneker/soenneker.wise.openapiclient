...
namespace Soenneker.Wise.OpenApiClient.Cases.Item
{
    ...
    public partial class WithCaseItemRequestBuilder : BaseRequestBuilder
    {
        ...
        /// <summary>
        /// Retrieves the complete details of a support case including all public messagesand reference information.#### ResponseReturns the full case object with:- Case metadata (id, type, subType, status, timestamps)- Profile and external reference information- All public messages associated with the caseMessages in the response can have four different content structures:1. Attributes Content (REQUEST messages)Present in messages like `DEPOSIT_SANCTION_HIT_REQUEST`, `REFERENCE_SANCTION_HIT_REQUEST`, etc.Contains dynamic attributes that define what information is required.2. Submission Data Content (SUBMISSION messages)Present in messages like `DEPOSIT_SANCTION_HIT_SUBMISSION`, `REFERENCE_LOCATION_HIT_SUBMISSION`, etc.Contains the data submitted by the partner in response to a request.3. Text Content (FREEFORM messages)Present in `FREEFORM` messages. Contains free text conversation between partners and Wise.4. File Submission Content (FILE_SUBMISSION messages)Present in `FILE_SUBMISSION` messages. Contains file references (`profileId` and `fileIds`) for files attached to the case by the partner.
        /// </summary>
